//
//  RectAxisOffsets.swift
//  UIKitPlus
//
//  Created by Peter Meyers on 11/16/19.
//  Copyright © 2019 Peter Meyers. All rights reserved.

import UIKit

public enum ConstraintConvertable: Equatable {
...
extension Array where Element == ConstraintConvertable {
    ...
    public static func allEdges(inset: CGFloat, from layoutGuide: LayoutGuide = .view) -> [ConstraintConvertable] {
        allEdges(inset: UIEdgeInsets(top: inset, left: inset, bottom: inset, right: inset), from: layoutGuide)
    }

    public static func allEdges(inset: UIEdgeInsets, from layoutGuide: LayoutGuide = .view) -> [ConstraintConvertable] {
        [
            .top(toLayoutGuide: layoutGuide, offset: inset.top),
            .bottom(toLayoutGuide: layoutGuide, offset: -inset.bottom),
            .leading(toLayoutGuide: layoutGuide, offset: inset.left),
            .trailing(toLayoutGuide: layoutGuide, offset: -inset.right)
        ]
    }
}
